...
image:https://github.com/carvvf/unoserver-docker/actions/workflows/scan-active-image.yml/badge.svg?branch=custom["Weekly Trivy scan status",link="https://github.com/carvvf/unoserver-docker/actions/workflows/scan-active-image.yml"]

NOTE: The information below refers to the upstream `unoconv/unoserver-docker` repository and may not fully reflect this fork. In this branch `custom`, `Dockerfile` is intentionally kept aligned with upstream. Use `Dockerfile.custom` for branch-specific hardened builds.
